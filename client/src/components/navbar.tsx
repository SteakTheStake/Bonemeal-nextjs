...
export function Navbar() {
  ...
  return (
    <nav className="sticky top-0 z-50 w-full border-b border-white/10 bg-white/5 backdrop-blur-xl">
                  ...
                  <Button variant="ghost" className="relative h-10 w-10 rounded-full">
                    <Avatar className="h-10 w-10 border-2 border-green-500/20">
                      <AvatarImage 
                        ...
                        alt={user.username} 
                      />
                      <AvatarFallback className="bg-gradient-to-br from-green-500/20 to-emerald-600/20">
                        {user.username?.charAt(0).toUpperCase()}
                      </AvatarFallback>
                    </Avatar>
                  </Button>
                </DropdownMenuTrigger>
                <DropdownMenuContent className="w-56 bg-zinc-950/95 backdrop-blur-xl border-white/10" align="end">
                  <div className="flex items-center justify-start gap-2 p-2">
                    ...
                    <div className="flex flex-col space-y-1">
                      ...
                      {user.discriminator && user.discriminator !== "0" && (
                        <p className="text-xs text-zinc-400">#{user.discriminator}</p>
                      )}
                    </div>
                  </div>
                  <DropdownMenuSeparator className="bg-white/10" />
                  <DropdownMenuItem className="text-zinc-400 hover:text-white hover:bg-white/5">
